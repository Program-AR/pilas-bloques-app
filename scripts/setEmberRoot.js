...
if (!indexPath)
  throw "You must pass the ember build folder path as a parameter to this script"
indexPath += "/index.html"

let rootURL = process.argv[3]
if (rootURL === undefined)
  throw "You must pass the root URL of the ember project to this script"
...
const metaContents = oldIndex
  .split(`<meta name="pilasbloques/config/environment" content=`)[1]
  .split(`/>`)[0]
// Replacing environment configuration with new
var newIndex = oldIndex.replace(
  metaContents,
  JSON.stringify(replaceApiUrl(JSON.parse(metaContents)))
)
