...
export namespace LocalStorage {
    ...
    export const getSelectedLanguageKey = (): LanguageCode | null => _get(SELECTED_LANGUAGE_KEY, false)
    ...
    export const saveSelectedLanguageKey = (key: LanguageCode) => _save(SELECTED_LANGUAGE_KEY, key, false)
    ...
    const _get = (key: string, shouldParse: boolean = true) => _doSafe((storage: Storage) => {
        const value = storage.getItem(key)
        return value && shouldParse ? JSON.parse(value) : value
    })

    const _save = (key: string, data: any = null, shouldStringify: boolean = true) => { _doSafe((storage: Storage) => storage.setItem(key, (shouldStringify ? JSON.stringify(data) : data))) }
